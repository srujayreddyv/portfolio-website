import { SkillCategory } from '@/types'

export const skillCategories: SkillCategory[] = [
  {
    category: "Backend & APIs",
    skills: [
      { name: "Python", level: "Expert", yearsOfExperience: 6 },
      { name: "FastAPI", level: "Expert", yearsOfExperience: 4 },
      { name: "Django", level: "Advanced", yearsOfExperience: 5 },
      { name: "Node.js", level: "Advanced", yearsOfExperience: 4 },
      { name: "Go", level: "Intermediate", yearsOfExperience: 2 },
      { name: "ASP.NET Core", level: "Advanced", yearsOfExperience: 3 },
      { name: "PHP", level: "Intermediate", yearsOfExperience: 2 },
      { name: "C#", level: "Advanced", yearsOfExperience: 3 },
      { name: "Flask", level: "Advanced", yearsOfExperience: 3 },
      { name: "Express", level: "Advanced", yearsOfExperience: 4 },
      { name: "REST APIs", level: "Expert", yearsOfExperience: 6 },
      { name: "GraphQL", level: "Advanced", yearsOfExperience: 3 },
      { name: "WebSockets", level: "Advanced", yearsOfExperience: 3 },
      { name: "gRPC", level: "Intermediate", yearsOfExperience: 2 },
      { name: "Git", level: "Expert", yearsOfExperience: 6 },
      { name: "Postman", level: "Expert", yearsOfExperience: 5 },
      { name: "Swagger/OpenAPI", level: "Advanced", yearsOfExperience: 4 },
      { name: "Entity Framework", level: "Advanced", yearsOfExperience: 3 },
      { name: "Django REST Framework", level: "Expert", yearsOfExperience: 5 }
    ]
  },
  {
    category: "AI Systems & GenAI Engineering",
    skills: [
      { name: "RAG Pipelines", level: "Advanced", yearsOfExperience: 2 },
      { name: "AWS Bedrock", level: "Advanced", yearsOfExperience: 2 },
      { name: "LangChain", level: "Advanced", yearsOfExperience: 2 },
      { name: "Hugging Face Transformers", level: "Advanced", yearsOfExperience: 3 },
      { name: "PyTorch", level: "Advanced", yearsOfExperience: 3 },
      { name: "TensorFlow", level: "Intermediate", yearsOfExperience: 2 },
      { name: "CrewAI", level: "Advanced", yearsOfExperience: 1 },
      { name: "Scikit-learn", level: "Advanced", yearsOfExperience: 4 },
      { name: "FAISS", level: "Advanced", yearsOfExperience: 2 },
      { name: "Pinecone", level: "Advanced", yearsOfExperience: 2 },
      { name: "OpenSearch", level: "Intermediate", yearsOfExperience: 1 },
      { name: "MLflow", level: "Advanced", yearsOfExperience: 2 }
    ]
  },
  {
    category: "Cloud & Infrastructure",
    skills: [
      { name: "AWS", level: "Expert", yearsOfExperience: 5 },
      { name: "Azure", level: "Advanced", yearsOfExperience: 3 },
      { name: "GCP", level: "Intermediate", yearsOfExperience: 2 },
      { name: "Docker", level: "Expert", yearsOfExperience: 4 },
      { name: "Kubernetes", level: "Advanced", yearsOfExperience: 3 },
      { name: "Terraform", level: "Advanced", yearsOfExperience: 3 },
      { name: "Azure DevOps", level: "Advanced", yearsOfExperience: 3 },
      { name: "Grafana", level: "Intermediate", yearsOfExperience: 2 },
      { name: "Prometheus", level: "Intermediate", yearsOfExperience: 2 },
      { name: "Datadog", level: "Intermediate", yearsOfExperience: 2 },
      { name: "GitHub Actions", level: "Expert", yearsOfExperience: 4 },
      { name: "CloudWatch", level: "Advanced", yearsOfExperience: 3 }
    ]
  },
  {
    category: "Data Platforms",
    skills: [
      { name: "PostgreSQL", level: "Expert", yearsOfExperience: 6 },
      { name: "DynamoDB", level: "Advanced", yearsOfExperience: 3 },
      { name: "MS SQL Server", level: "Advanced", yearsOfExperience: 4 },
      { name: "Redis", level: "Advanced", yearsOfExperience: 3 },
      { name: "Apache Kafka", level: "Advanced", yearsOfExperience: 3 },
      { name: "Snowflake", level: "Intermediate", yearsOfExperience: 2 },
      { name: "Amazon Redshift", level: "Intermediate", yearsOfExperience: 2 },
      { name: "Power BI", level: "Advanced", yearsOfExperience: 3 },
      { name: "Apache Airflow", level: "Advanced", yearsOfExperience: 3 },
      { name: "Apache Spark", level: "Intermediate", yearsOfExperience: 2 },
      { name: "Elasticsearch", level: "Advanced", yearsOfExperience: 3 },
      { name: "Tableau", level: "Intermediate", yearsOfExperience: 2 },
      { name: "Amazon QuickSight", level: "Intermediate", yearsOfExperience: 2 }
    ]
  },
  {
    category: "Frontend",
    skills: [
      { name: "React", level: "Expert", yearsOfExperience: 5 },
      { name: "Next.js", level: "Advanced", yearsOfExperience: 3 },
      { name: "TypeScript", level: "Expert", yearsOfExperience: 4 },
      { name: "JavaScript", level: "Expert", yearsOfExperience: 6 },
      { name: "React Native", level: "Advanced", yearsOfExperience: 3 },
      { name: "Nuxt.js", level: "Advanced", yearsOfExperience: 2 },
      { name: "Angular", level: "Intermediate", yearsOfExperience: 2 },
      { name: "Bootstrap", level: "Advanced", yearsOfExperience: 4 },
      { name: "Material UI", level: "Advanced", yearsOfExperience: 3 },
      { name: "D3.js", level: "Intermediate", yearsOfExperience: 2 },
      { name: "Leaflet.js", level: "Advanced", yearsOfExperience: 3 },
      { name: "GeoServer", level: "Intermediate", yearsOfExperience: 2 },
      { name: "PostGIS", level: "Advanced", yearsOfExperience: 4 }
    ]
  }
]
