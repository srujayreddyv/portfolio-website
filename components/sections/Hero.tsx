...
export default function Hero() {
  return (
    ...
    >
      {/* Grid hairline overlay — center divider, desktop only (it sits in the
          gutter between the two columns; on mobile a single column it would
          just bisect the content, so hide it below lg). */}
      <div
        aria-hidden="true"
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-20 relative">
        <div className="grid grid-cols-1 lg:grid-cols-[1.15fr_1fr] gap-10 lg:gap-14 items-start">
          ...
          <div className="space-y-6 sm:space-y-8">
            ...
            {/* Identity row: photo + name side-by-side.
                Photo is 80-96px (HeroImageButton compact), heading caps at
                text-5xl so the name fits on one line alongside the photo. */}
            <div className="flex items-center gap-4 sm:gap-5">
              ...
              <div className="min-w-0 flex-1">
                <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-5xl font-bold text-ink leading-[1.05] tracking-[-0.02em]">
                  {personalData.name}
                </h1>
              </div>
            </div>

            {/* Subtitle + heroLead — separate block beneath the identity row */}
