...
import { Code, Database, Cloud, ChevronDown } from 'lucide-react';
import {
  SiAmazoncloudwatch,
  SiAmazonwebservices,
  SiAngular,
...
/**
 * SkillCategory — Direction 2 mono rows.
 *
 * Hairline-bordered card; each skill is a compact mono row (icon · name) with
 * a right-aligned proficiency tag (Expert in accent, Advanced in ink-70). No
 * year counts (per design direction).
 *
 * Disclosure: controlled useState "show more" — ported from the main-branch
 * refactor. Shows maxInitialSkills initially; expands to the full list.
 */
const SkillCategory: React.FC<SkillCategoryProps> = ({ category, isPrimary = false }) => {
  ...
  const displayedSkills = isExpanded
    ? category.skills
    : category.skills.slice(0, maxInitialSkills);
  ...
  return (
    ...
    >
      ...
      {/* Show more / less — controlled disclosure */}
      {hasMoreSkills && (
        <div className="mt-4 pt-3 border-t border-hairline">
          <button
            ...
            className="w-full inline-flex items-center justify-center gap-1.5 font-mono text-[11px] sm:text-xs text-muted hover:text-accent transition-colors duration-150"
          >
            <span>
              {isExpanded
                ? 'show less'
                : `show ${category.skills.length - maxInitialSkills} more`}
            </span>
            <ChevronDown
